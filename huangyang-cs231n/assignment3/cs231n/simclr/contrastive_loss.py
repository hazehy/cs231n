...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        sim_k_k_N = sim(z_k, z_k_N)
        numerator = torch.exp(sim_k_k_N / tau)
        denom1, denom2 = 0, 0
        
        for m in range(2 * N):
            z_m = out[m]
            denom1 += torch.exp(sim(z_k, z_m) / tau) if m != k else 0
            denom2 += torch.exp(sim(z_k_N, z_m) / tau) if m != k+N else 0

        total_loss += - torch.log(numerator / denom1)
        total_loss += - torch.log(numerator / denom2)
...
def sim_positive_pairs(out_left, out_right):
    ...
    dot_row = torch.einsum('ij,ij->i', out_left, out_right)
    norm_row = torch.linalg.norm(out_left, dim=1) * torch.linalg.norm(out_right, dim=1)
    pos_pairs = (dot_row / norm_row).unsqueeze(1)
...
def compute_sim_matrix(out):
    ...
    dot_matrix = torch.einsum('ik,jk->ij', out, out)
    norm_row = torch.linalg.norm(out, dim=1, keepdim=True)
    norm_matrix = norm_row @ norm_row.T
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    numerator = torch.exp(sim_pos_pairs / tau).repeat(2, 1)
