...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        cap_embeddings = self.embedding(captions)
        cap_embeddings = self.positional_encoding(cap_embeddings)

        img_embeddings = self.visual_projection(features) 
        img_embeddings = img_embeddings.unsqueeze(1)
        tgt_mask = torch.tril(torch.ones(T, T))

        decoder_output = self.transformer(tgt=cap_embeddings, memory=img_embeddings, tgt_mask=tgt_mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        encoder_output = self.transformer(patches)
        features = torch.mean(encoder_output, dim=1)
