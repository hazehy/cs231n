...
def get_similarity_no_loop(text_features, image_features):
    ...
    text_norm = text_features / torch.linalg.norm(text_features,dim=1,keepdim=True)
    image_norm = image_features / torch.linalg.norm(image_features,dim=1,keepdim=True)
...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    max_sim_indices = torch.argmax(similarity, dim=0)
    pred_classes = [class_texts[i] for i in max_sim_indices.tolist()]
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        images_tensor = torch.cat(processed_images, dim=0).to(device)
        image_features = clip_model.encode_image(images_tensor)
        image_features_norm = image_features / torch.linalg.norm(image_features,dim=1,keepdim=True)
        self.image_features_norm = image_features_norm
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        text_features_norm = text_features / torch.linalg.norm(text_features,dim=1,keepdim=True)
        image_features_norm = self.image_features_norm
