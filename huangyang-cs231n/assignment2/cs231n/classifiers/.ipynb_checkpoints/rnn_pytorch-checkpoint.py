...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        captions_out = captions_out.long()
        ...
        h0 = torch.mm(features, W_proj) + b_proj
        x = word_embedding_forward(captions_in, W_embed)
        if self.cell_type == 'rnn':
            h = rnn_forward(x, h0, Wx, Wh, b)
        else:
            raise ValueError('LSTM not implemented yet')
            
        N, T, H = h.shape
        h_reshaped = h.reshape(N * T, H)
        scores = torch.mm(h_reshaped, W_vocab) + b_vocab
    ...
    def sample(self, features, max_length=30):
        ...
        h = torch.mm(features, W_proj) + b_proj
        current_word = torch.tensor([self._start] * N, dtype=torch.long, device=features.device)
        for t in range(max_length):
            x_t = W_embed[current_word]
            if self.cell_type == 'rnn':
                h = rnn_step_forward(x_t, h, Wx, Wh, b)
            else:
                raise ValueError('LSTM not implemented yet')
            scores = torch.mm(h, W_vocab) + b_vocab
