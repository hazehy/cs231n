...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # 计算输入到隐藏层的变换
    input_transform = torch.mm(x, Wx)
    
    # 计算隐藏层到隐藏层的变换  
    hidden_transform = torch.mm(prev_h, Wh)
    
    # 激活函数
    next_h = torch.tanh(input_transform + hidden_transform + b)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        ...
        # 使用单步前向传播函数计算当前时间步的隐藏状态
        next_h = rnn_step_forward(x_t, prev_h, Wx, Wh, b)
        
        # 存储当前时间步的隐藏状态
        h[:, t, :] = next_h
        
        # 更新prev_h为下一个时间步的初始隐藏状态
        prev_h = next_h
